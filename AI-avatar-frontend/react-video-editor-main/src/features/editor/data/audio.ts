...
// Function to add new audio to the list
export const addAudioToList = (audioUrl: string, text: string) => {
  const newAudio: Partial<IAudio> = {
    id: `script-${Date.now()}`,
    details: {
      src: audioUrl,
    },
  ...
  audioList = [newAudio, ...audioList];
  return newAudio;
};

// Export the list as a getter to always get the latest version
export const getAudioList = () => audioList;
